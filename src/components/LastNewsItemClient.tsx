                ...
                <Link href={`/news/${item.slug || item.id}`}>
                  <div className="flex items-center gap-0">
                    ...
                    <CardContent className="flex w-1/2 flex-col justify-center order-2 md:order-2">
                      ...
                      {/* عنوان */}
                      <h2 className="text-xl leading-9 text-blue-dark group-hover::text-brand transition-colors font-black">
                        {item.title}
                      </h2>
          ...
          {news.map((_, index) => (
            <button
              ...
              className={`transition-all duration-300 rounded-full ${
                current === index ? 'w-8 h-3 bg-slate-300' : 'w-3 h-3 bg-brand-dark hover:bg-brand'
              }`}
